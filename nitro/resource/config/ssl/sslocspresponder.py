...
class sslocspresponder(base_resource) :
	...
	@property
	def url(self) :
		r"""URL of the OCSP responder.<br/>Minimum length =  1.
		"""
		try :
			return self._url
		except Exception as e:
	...
	@property
	def cache(self) :
		r"""Enable caching of responses. Caching of responses received from the OCSP responder enables faster responses to the clients and reduces the load on the OCSP responder.<br/>Possible values = ENABLED, DISABLED.
		"""
		try :
			return self._cache
		except Exception as e:
	...
	def cachetimeout(self) :
		r"""Timeout for caching the OCSP response. After the timeout, the NetScaler sends a fresh request to the OCSP responder for the certificate status. If a timeout is not specified, the timeout provided in the OCSP response applies.<br/>Default value: 1<br/>Minimum length =  1<br/>Maximum length =  1440.
		"""
		try :
	...
	def batchingdelay(self) :
		r"""Maximum time, in milliseconds, to wait to accumulate OCSP requests to batch.  Does not apply if the Batching Depth is 1.<br/>Maximum length =  10000.
		"""
		try :
	...
	def resptimeout(self) :
		r"""Time, in milliseconds, to wait for an OCSP response. When this time elapses, an error message appears or the transaction is forwarded, depending on the settings on the virtual server. Includes Batching Delay time.<br/>Maximum length =  120000.
		"""
		try :
	...
	@property
	def useaia(self) :
		r"""Only use the URL present in the certificate.
		"""
		try :
			return self._useaia
		except Exception as e:
	...
	@property
	def dns(self) :
		r"""Was DNS resolution successful for a domain-based OCSP responder.
		"""
		try :
			return self._dns
		except Exception as e:
	...
	@property
	def ipaddress(self) :
		r"""The IPv6 address of the ocsp responder.
		"""
		try :
