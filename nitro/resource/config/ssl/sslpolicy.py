...
class sslpolicy(base_resource) :
	...
	@property
	def rule(self) :
		r"""Expression, against which traffic is evaluated. Written in the classic or default syntax.
		Note:
	...
	@property
	def action(self) :
		r"""Name of the built-in or user-defined action to perform on the request. Available built-in actions are NOOP, RESET, DROP, CLIENTAUTH, and NOCLIENTAUTH.
		"""
		try :
			return self._action
		except Exception as e:
	...
	def undefaction(self) :
		r"""Name of the action to be performed when the result of rule evaluation is undefined. Possible values for control policies: CLIENTAUTH, NOCLIENTAUTH, NOOP, RESET, DROP. Possible values for data policies: NOOP, RESET or DROP.
		"""
		try :
