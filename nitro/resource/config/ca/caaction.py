...
class caaction(base_resource) :
	...
	def accumressize(self) :
		r"""Size of the data, in KB, that the server must respond with. The NetScaler uses this data to compute a hash which is then used to lookup within the T2100 appliance.
		"""
		try :
	...
	@property
	def type(self) :
		r"""Specifies whether the NetScaler must lookup for the response on the T2100 appliance or serve the response directly from the server.<br/>Possible values = nolookup, lookup, noop.
		"""
		try :
			return self._type
		except Exception as e:
	...
	@property
	def builtin(self) :
		r"""Flag to determine whether content accelerator action is built-in or not.<br/>Possible values = MODIFIABLE, DELETABLE, IMMUTABLE, PARTITION_ALL.
		"""
		try :
