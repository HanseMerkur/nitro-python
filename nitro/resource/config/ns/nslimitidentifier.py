...
class nslimitidentifier(base_resource) :
	...
	def selectorname(self) :
		r"""Name of the rate limit selector. If this argument is NULL, rate limiting will be applied on all traffic received by the virtual server or the NetScaler (depending on whether the limit identifier is bound to a virtual server or globally) without any filtering.<br/>Minimum length =  1.
		"""
		try :
