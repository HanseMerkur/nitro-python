...
class rewriteaction(base_resource) :
	...
	@property
	def target(self) :
		r"""Default syntax expression that specifies which part of the request or response to rewrite.<br/>Minimum length =  1.
		"""
		try :
			return self._target
		except Exception as e:
	...
	def stringbuilderexpr(self) :
		r"""Default syntax expression that specifies the content to insert into the request or response at the specified location, or that replaces the specified string.
		"""
		try :
	...
	@property
	def builtin(self) :
		r"""Flag to determine whether rewrite action is built-in or not.<br/>Possible values = MODIFIABLE, DELETABLE, IMMUTABLE, PARTITION_ALL.
		"""
		try :
