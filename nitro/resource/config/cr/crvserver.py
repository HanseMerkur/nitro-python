...
class crvserver(base_resource) :
	...
	def redirecturl(self) :
		r"""URL of the server to which to redirect traffic if the cache redirection virtual server configured on the NetScaler appliance becomes unavailable.<br/>Minimum length =  1<br/>Maximum length =  128.
		"""
		try :
	...
	@property
	def backendssl(self) :
		r"""Decides whether the backend connection made by NS to the origin server will be HTTP or SSL. Applicable only for SSL type CR Forward proxy vserver.<br/>Default value: DISABLED<br/>Possible values = ENABLED, DISABLED.
		"""
		try :
