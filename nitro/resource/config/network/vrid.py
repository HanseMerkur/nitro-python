...
class vrid(base_resource) :
	...
	@property
	def ownernode(self) :
		r"""Assign a cluster node as the owner of this VMAC address. If no owner is configured, owner node is displayed as ALL and one node is dynamically elected as the owner.<br/>Maximum length =  31.
		"""
		try :
	...
	def trackifnumpriority(self) :
		r"""Priority by which the Effective priority will be reduced if any of the tracked interfaces goes down.<br/>Default value: 0<br/>Minimum length =  1<br/>Maximum length =  255.
		"""
		try :
	...
	def preemptiondelaytimer(self) :
		r"""Preemption delay time in seconds. If any high priority node will come in network, it will wait for these many seconds before becoming master.<br/>Default value: 0<br/>Minimum length =  1<br/>Maximum length =  36000.
		"""
		try :
	...
	@property
	def all(self) :
		r"""Remove all the configured VMAC addresses from the NetScaler appliance.
		"""
		try :
			return self._all
		except Exception as e:
