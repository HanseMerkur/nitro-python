...
class lbmonitor(base_resource) :
	...
	@property
	def scriptname(self) :
		r"""Path and name of the script to execute. The script must be available on the NetScaler appliance, in the /nsconfig/monitors/ directory.<br/>Minimum length =  1.
		"""
		try :
	...
	@property
	def deviation(self) :
		r"""Time value added to the learned average response time in dynamic response time monitoring (DRTM). When a deviation is specified, the appliance learns the average response time of bound services and adds the deviation to the average. The final value is then continually adjusted to accommodate response time variations over time. Specified in milliseconds, seconds, or minutes.<br/>Maximum length =  20939000.
		"""
		try :
	...
	@property
	def interval(self) :
		r"""Time interval between two successive probes. Must be greater than the value of Response Time-out.<br/>Default value: 5<br/>Minimum length =  1<br/>Maximum length =  20940000.
		"""
		try :
	...
	@property
	def downtime(self) :
		r"""Time duration for which to wait before probing a service that has been marked as DOWN. Expressed in milliseconds, seconds, or minutes.<br/>Default value: 30<br/>Minimum length =  1<br/>Maximum length =  20939000.
		"""
		try :
	...
	@property
	def evalrule(self) :
		r"""Default syntax expression that evaluates the database server's response to a MYSQL-ECV or MSSQL-ECV monitoring query. Must produce a Boolean result. The result determines the state of the server. If the expression returns TRUE, the probe succeeds. 
