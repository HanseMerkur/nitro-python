...
class lbvserver(base_resource) :
	...
	def servicetype(self) :
		r"""Protocol used by the service (also called the service type).<br/>Possible values = HTTP, FTP, TCP, UDP, SSL, SSL_BRIDGE, SSL_TCP, DTLS, NNTP, DNS, DHCPRA, ANY, SIP_UDP, SIP_TCP, SIP_SSL, DNS_TCP, RTSP, PUSH, SSL_PUSH, RADIUS, RDP, MYSQL, MSSQL, DIAMETER, SSL_DIAMETER, TFTP, ORACLE, SMPP, SYSLOGTCP, SYSLOGUDP, FIX.
		"""
		try :
	...
	@property
	def port(self) :
		r"""Port number for the virtual server.<br/>Range 1 - 65535.
		"""
		try :
			return self._port
		except Exception as e:
	...
	def backuplbmethod(self) :
		r"""Backup load balancing method. Becomes operational if the primary load balancing method fails or cannot be used.
	...
	@property
	def resrule(self) :
		r"""Default syntax expression specifying which part of a server's response to use for creating rule based persistence sessions (persistence type RULE). Can be either an expression or the name of a named expression.
		Example:
	...
	def icmpvsrresponse(self) :
		r"""How the NetScaler appliance responds to ping requests received for an IP address that is common to one or more virtual servers. Available settings function as follows:
