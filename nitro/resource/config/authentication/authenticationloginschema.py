...
class authenticationloginschema(base_resource) :
	...
	@property
	def name(self) :
		r"""Name for the new login schema. Must begin with an ASCII alphanumeric or underscore (_) character, and must contain only ASCII alphanumeric, underscore, hash (#), period (.), space, colon (:), at (@), equals (=), and hyphen (-) characters. Cannot be changed after an action is created.
	...
	def userexpression(self) :
		r"""Expression for username extraction during login.<br/>Minimum length =  1.
		"""
		try :
	...
	def passwdexpression(self) :
		r"""Expression for password extraction during login.<br/>Minimum length =  1.
		"""
		try :
