...
class vpnparameter(base_resource) :
	...
	@property
	def winsip(self) :
		r"""WINS server IP address to add to NetScaler Gateway for name resolution.<br/>Minimum length =  1.
		"""
		try :
			return self._winsip
		except Exception as e:
	...
	def clientoptions(self) :
		r"""Display only the configured menu options when you select the "Configure NetScaler Gateway" option in the NetScaler Gateway Plug-in's system tray icon for Windows.<br/>Possible values = none, all, services, filetransfer, configuration.
		"""
		try :
	...
	def clientconfiguration(self) :
		r"""Display only the configured tabs when you select the "Configure NetScaler Gateway" option in the NetScaler Gateway Plug-in's system tray icon for Windows.<br/>Possible values = none, all, general, tunnel, trace, compression.
		"""
		try :
	...
	@property
	def sso(self) :
		r"""Set single sign-on (SSO) for the session. When the user accesses a server, the user's logon credentials are passed to the server for authentication.<br/>Default value: OFF<br/>Possible values = ON, OFF.
		"""
		try :
			return self._sso
		except Exception as e:
	...
	@property
	def icaproxy(self) :
		r"""Enable ICA proxy to configure secure Internet access to servers running Citrix XenApp or XenDesktop by using Citrix Receiver instead of the NetScaler Gateway Plug-in.<br/>Default value: OFF<br/>Possible values = ON, OFF.
		"""
		try :
	...
	def securebrowse(self) :
		r"""Allow users to connect through NetScaler Gateway to network resources from iOS and Android mobile devices with Citrix Receiver. Users do not need to establish a full VPN tunnel to access resources in the secure network.<br/>Default value: ENABLED<br/>Possible values = ENABLED, DISABLED.
		"""
		try :
