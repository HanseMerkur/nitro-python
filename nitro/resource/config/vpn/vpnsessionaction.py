...
class vpnsessionaction(base_resource) :
	...
	@property
	def name(self) :
		r"""Name for the NetScaler Gateway profile (action). Must begin with an ASCII alphabetic or underscore (_) character, and must consist only of ASCII alphanumeric, underscore, hash (#), period (.), space, colon (:), at (@), equals (=), and hyphen (-) characters. Cannot be changed after the profile is created.
	...
	@property
	def winsip(self) :
		r"""WINS server IP address to add to NetScaler Gateway for name resolution.
		"""
		try :
			return self._winsip
		except Exception as e:
	...
	@property
	def splitdns(self) :
		r"""Route the DNS requests to the local DNS server configured on the user device, or NetScaler Gateway (remote), or both.<br/>Possible values = LOCAL, REMOTE, BOTH.
		"""
		try :
	...
	def clientsecurity(self) :
		r"""Specify the client security check for the user device to permit a NetScaler Gateway session. The web address or IP address is not included in the expression for the client security check.
		"""
		try :
	...
	def authorizationgroup(self) :
		r"""Comma-separated list of groups in which the user is placed when none of the groups that the user is a part of is configured on NetScaler Gateway. The authorization policy can be bound to these groups to control access to the resources.<br/>Minimum length =  1.
		"""
		try :
	...
	@property
	def smartgroup(self) :
		r"""This is the group in which the user is placed when the sessionpolicy associated with this sessionaction succeeds. The vpn session policy will do the post auth EPA check and if the check succeeds the user is placed in the group specified with smartgroup. The is_member_of (http.req.user.is_member_of) expression can then be used with policies to check if EPA has passed on the user belonging to this smartgroup.<br/>Minimum length =  1<br/>Maximum length =  64.
		"""
		try :
	...
	def clientidletimeout(self) :
		r"""Time, in minutes, after which to time out the user session if NetScaler Gateway does not detect mouse or keyboard activity.<br/>Minimum length =  1<br/>Maximum length =  9999.
		"""
		try :
	...
	def allprotocolproxy(self) :
		r"""IP address of the proxy server to use for all protocols supported by NetScaler Gateway.<br/>Minimum length =  1.
		"""
		try :
	...
	def clientoptions(self) :
		r"""Display only the configured menu options when you select the "Configure NetScaler Gateway" option in the NetScaler Gateway Plug-in system tray icon for Windows.<br/>Possible values = none, all, services, filetransfer, configuration.
		"""
		try :
	...
	def clientconfiguration(self) :
		r"""Display only the configured tabs when you select the "Configure NetSCaler Gateway" option in the NetScaler Gateway Plug-in system tray icon for Windows.<br/>Possible values = none, all, general, tunnel, trace, compression.
		"""
		try :
	...
	@property
	def sso(self) :
		r"""Set single sign-on (SSO) for the session. When the user accesses a server, the user's logon credentials are passed to the server for authentication.<br/>Possible values = ON, OFF.
		"""
		try :
			return self._sso
		except Exception as e:
	...
	@property
	def usemip(self) :
		r"""Enable or disable the use of a unique IP address alias, or a mapped IP address, as the client IP address for each client session. Allow NetScaler Gateway to use the mapped IP address as an intranet IP address when all other IP addresses are not available. 
	...
	def clientdebug(self) :
		r"""Set the trace level on NetScaler Gateway. Technical support technicians use these debug logs for in-depth debugging and troubleshooting purposes. Available settings function as follows: 
	...
	@property
	def homepage(self) :
		r"""Web address of the home page that appears when users log on. Otherwise, users receive the default home page for NetScaler Gateway, which is the Access Interface.
		"""
		try :
	...
	@property
	def icaproxy(self) :
		r"""Enable ICA proxy to configure secure Internet access to servers running Citrix XenApp or XenDesktop by using Citrix Receiver instead of the NetScaler Gateway Plug-in.<br/>Possible values = ON, OFF.
		"""
		try :
	...
	def citrixreceiverhome(self) :
		r"""Web address for the Citrix Receiver home page. Configure NetScaler Gateway so that when users log on to the appliance, the NetScaler Gateway Plug-in opens a web browser that allows single sign-on to the Citrix Receiver home page.
		"""
		try :
	...
	def iipdnssuffix(self) :
		r"""An intranet IP DNS suffix. When a user logs on to NetScaler Gateway and is assigned an IP address, a DNS record for the user name and IP address combination is added to the NetScaler Gateway DNS cache. You can configure a DNS suffix to append to the user name when the DNS record is added to the cache. You can reach to the host from where the user is logged on by using the user's name, which can be easier to remember than an IP address. When the user logs off from NetScaler Gateway, the record is removed from the DNS cache.<br/>Minimum length =  1.
		"""
		try :
	...
	def forcedtimeout(self) :
		r"""Force a disconnection from the NetScaler Gateway Plug-in with NetScaler Gateway after a specified number of minutes. If the session closes, the user must log on again.<br/>Minimum length =  1<br/>Maximum length =  65535.
		"""
		try :
	...
	def clientlessvpnmode(self) :
		r"""Enable clientless access for web, XenApp or XenDesktop, and FileShare resources without installing the NetScaler Gateway Plug-in. Available settings function as follows: 
	...
	def allowedlogingroups(self) :
		r"""Specify groups that have permission to log on to NetScaler Gateway. Users who do not belong to this group or groups are denied access even if they have valid credentials.<br/>Minimum length =  1<br/>Maximum length =  511.
		"""
		try :
	...
	def securebrowse(self) :
		r"""Allow users to connect through NetScaler Gateway to network resources from iOS and Android mobile devices with Citrix Receiver. Users do not need to establish a full VPN tunnel to access resources in the secure network.<br/>Possible values = ENABLED, DISABLED.
		"""
		try :
