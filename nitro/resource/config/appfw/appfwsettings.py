...
class appfwsettings(base_resource) :
	...
	def defaultprofile(self) :
		r"""Profile to use when a connection does not match any policy. Default setting is APPFW_BYPASS, which sends unmatched connections back to the NetScaler appliance without attempting to filter them further.<br/>Default value: APPFW_BYPASS<br/>Minimum length =  1.
		"""
		try :
	...
	def importsizelimit(self) :
		r"""Cumulative total maximum number of bytes in web forms imported to a protected web site. If a user attempts to upload files with a total byte count higher than the specified limit, the application firewall blocks the request.<br/>Default value: 134217728<br/>Minimum length =  1<br/>Maximum length =  134217728.
		"""
		try :
