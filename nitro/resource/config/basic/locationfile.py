...
class locationfile(base_resource) :
	...
	def Locationfile(self) :
		r"""Name of the location file, with or without absolute path. If the path is not included, the default path (/var/netscaler/locdb) is assumed. In a high availability setup, the static database must be stored in the same location on both NetScaler appliances.<br/>Minimum length =  1.
		"""
		try :
