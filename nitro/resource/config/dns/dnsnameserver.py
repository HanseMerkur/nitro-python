...
class dnsnameserver(base_resource) :
	...
	def dnsvservername(self) :
		r"""Name of a DNS virtual server. Overrides any IP address-based name servers configured on the NetScaler appliance.<br/>Minimum length =  1.
		"""
		try :
	...
	@property
	def local(self) :
		r"""Mark the IP address as one that belongs to a local recursive DNS server on the NetScaler appliance. The appliance recursively resolves queries received on an IP address that is marked as being local. For recursive resolution to work, the global DNS parameter, Recursion, must also be set. 
	...
	@property
	def port(self) :
		r"""Port of the service.<br/>Range 1 - 65535.
		"""
		try :
			return self._port
		except Exception as e:
