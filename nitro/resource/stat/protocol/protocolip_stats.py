...
class protocolip_stats(base_resource) :
	...
	def iptotaddrlookupfail(self) :
		r"""IP address lookups performed by the NetScaler that have failed because the destination IP address of the packet does not match any of the NetScaler owned IP addresses.
		"""
		try :
	...
	def iptotbadtransport(self) :
		r"""Packets received in which the protocol specified in the IP header is unknown to the NetScaler.
		"""
		try :
	...
	def iptotfragpktsgen(self) :
		r"""Fragmented packets created by the NetScaler.
		"""
		try :
	...
	def iptotsuccreassembly(self) :
		r"""Fragmented IP packets successfully reassembled on the NetScaler.
		"""
		try :
	...
	def iptotunknowndstrcvd(self) :
		r"""Packets received in which the destination IP address was not reachable or not owned by the NetScaler.
		"""
		try :
	...
	def iptotaddrlookup(self) :
		r"""IP address lookups performed by the NetScaler. When a packet is received on a non-established session, the NetScaler checks if the destination IP address is one of the NetScaler owned IP addresses.
		"""
		try :
	...
	def iptotinvalidpacketsize(self) :
		r"""Total number of packets received by NetScaler with invalid IP packet size.
		"""
		try :
	...
	def iptotreassemblyattempt(self) :
		r"""IP packets that the NetScaler attempts to reassemble. If one of the fragments is missing, the whole packet is dropped.
		"""
		try :
