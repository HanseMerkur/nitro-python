...
class protocoltcp_stats(base_resource) :
	...
	def tcperrretransmit(self) :
		r"""TCP packets retransmitted. The NetScaler attempts to retransmit the packet up to seven times, after which it resets the other half of the TCP connection.
		"""
		try :
	...
	def tcpsurgequeuelen(self) :
		r"""Connections in the surge queue. When the NetScaler cannot open a connection to the server, for example when maximum connections have been reached, the NetScaler queues these requests.
		"""
		try :
	...
	def tcpcurserverconnestablished(self) :
		r"""Current server connections in the Established state, which indicates that data transfer can occur between the NetScaler and the server.
		"""
		try :
	...
	def tcperrforthretransmissions(self) :
		r"""Packets retransmitted four times by the NetScaler.
		"""
		try :
	...
	def tcperrsixthretransmissions(self) :
		r"""Packets retransmitted six times by the NetScaler.
		"""
		try :
	...
	def tcptotsynflush(self) :
		r"""SYN packets flushed on the NetScaler because of no response from the server for three or more seconds.
		"""
		try :
	...
	def tcperrfastretransmissions(self) :
		r"""TCP packets on which the NetScaler performs a fast retransmission in response to three duplicate acknowledgements or a partial acknowledgement.  The NetScaler assumes that the packet is lost and retransmits the packet before its time-out.
		"""
		try :
	...
	def tcperrcltretrasmit(self) :
		r"""Packets retransmitted by a client. This usually occurs because the acknowledgement from the NetScaler has not reached the client.
		"""
		try :
	...
	def tcptotsynprobe(self) :
		r"""Probes from the NetScaler to a server. The NetScaler sends a SYN packet to the server to check its availability and expects a SYN_ACK packet from the server before a specified response timeout.
		"""
		try :
	...
	def tcptotfinwaitclosed(self) :
		r"""Connections closed on the NetScaler because the number of connections in the TIME_WAIT state has exceeded the default value of 7000.
		"""
		try :
	...
	def tcptotsynheld(self) :
		r"""SYN packets held on the NetScaler that are waiting for a server connection.
		"""
		try :
	...
	def tcperrthirdretransmissions(self) :
		r"""Packets retransmitted three times by the NetScaler.
		"""
		try :
	...
	def tcperrsvrretrasmit(self) :
		r"""Packets retransmitted by a server. This usually occurs because the acknowledgement from the NetScaler has not reached the server.
		"""
		try :
	...
	def tcptotzombiepassivehalfclosecltconnflushed(self) :
		r"""Passive half-closed client connections that are flushed because the NetScaler has closed the connection and there has been no activity on the connection.
		"""
		try :
	...
	def tcptotclientconnopened(self) :
		r"""Client connections opened by the NetScaler since startup (after three-way handshake). This counter is reset when the NetScaler is restarted.
		"""
		try :
	...
	def tcptotserverconnopened(self) :
		r"""Server connections initiated by the NetScaler since startup. This counter is reset when the NetScaler is restarted.
		"""
		try :
	...
	def tcperrretransmitgiveup(self) :
		r"""Number of times NetScaler terminates a connection after retransmitting the packet seven times on that connection.Retrasnmission happens when recieving end doesn't acknowledges the packet.
		"""
		try :
	...
	def tcperrfingiveup(self) :
		r"""Connections that were timed out by the NetScaler because of not receiving the ACK packet after retransmitting the FIN packet four times.
		"""
		try :
	...
	def tcperrsvrhole(self) :
		r"""TCP holes created on a server connection. When out of order packets are received from a server, a hole is created on the NetScaler for each group of missing packets.
		"""
		try :
	...
	def tcperrsyngiveup(self) :
		r"""Attempts to establish a connection on the NetScaler that timed out.
		"""
		try :
	...
	def tcperrfirstretransmissions(self) :
		r"""Packets retransmitted once by the NetScaler.
		"""
		try :
	...
	def tcptotzombiepassivehalfclosesrvconnflushed(self) :
		r"""Passive half-closed server connections that are flushed because the NetScaler has closed the connection and there has been no activity on the connection.
		"""
		try :
	...
	def tcperrsecondretransmissions(self) :
		r"""Packets retransmitted twice by the NetScaler.
		"""
		try :
	...
	def tcperrpartialretrasmit(self) :
		r"""Partial packet retransmits by a client or server due to congestion on the connection. This usually occurs because the window advertised by the NetScaler is not big enough to hold the full packet.
		"""
		try :
	...
	def tcperrclthole(self) :
		r"""TCP holes created on a client connection. When out of order packets are received from a client, a hole is created on the NetScaler for each group of missing packets.
		"""
		try :
	...
	def tcpcurclientconnestablished(self) :
		r"""Current client connections in the Established state, which indicates that data transfer can occur between the NetScaler and the client.
		"""
		try :
	...
	def tcperrseventhretransmissions(self) :
		r"""Packets retransmitted seven times by the NetScaler. If this fails, the NetScaler terminates the connection.
		"""
		try :
	...
	def tcperrfifthretransmissions(self) :
		r"""Packets retransmitted five times by the NetScaler.
		"""
		try :
