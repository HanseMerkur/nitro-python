...
class cache_stats(base_resource) :
	...
	def cachepercent304hits(self) :
		r"""304 responses as a percentage of all responses that the NetScaler served.
		"""
		try :
	...
	def cachepercentbytehit(self) :
		r"""Bytes served from the cache divided by total bytes served to the client. If compression is On in the NetScaler, this ratio may not reflect the bytes served by the compression module. If the compression is Off, this ratio is the same as cachePercentOriginBandwidthSaved.
		"""
		try :
	...
	def cachetotresponsebytes(self) :
		r"""Total number of HTTP response bytes served by NetScaler from both the origin and the cache.
		"""
		try :
	...
	def cachemaxmemorykb(self) :
		r"""Largest amount of memory the NetScaler can dedicate to caching, up to 50% of available memory. A 0 value disables caching, but the caching module continues to run. .
		"""
		try :
	...
	def cacherecentpercentoriginbandwidthsaved(self) :
		r"""Bytes served from cache divided by total bytes served to client. This ratio can be greater than 1 because of the assumption that all compression has been done in the NetScaler.
		"""
		try :
	...
	def cachepercentoriginbandwidthsaved(self) :
		r"""Percentage of origin bandwidth saved, expressed as number of bytes served from the integrated cache divided by all bytes served. The assumption is that all compression is done in the NetScaler.
		"""
		try :
